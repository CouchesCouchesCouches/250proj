...
void Txc10::forwardMessage(cMessage *msg)
{
    ...
    int inputGates = gateSize("in");
    if(arrivalGate != nullptr and inputGates > 1) { // not a self message and there is more than one incoming gate
        int arrivalGateId = arrivalGate->getIndex();

        EV << "Arrived on Gate ID: " << arrivalGateId <<"\n";
        while(k == arrivalGateId) {
            EV << "Random k gate was same as arrival gate. Re-choosing output gate\n";
            k = intuniform(0, n-1);
        }
    }
