...
class BellStateAnalyzer : public cSimpleModule {
 private:
  ...
  double BSAsuccess_rate = 0.5 * 0.5 * 0.5;  // detector probability = 0.8
...
void BellStateAnalyzer::sendBSAresult(bool result, bool sendresults) {
  ...
  if (!sendresults) {
    ...
    //EV << "---------------SUCCESS OVER TOTAL:  " << DEBUG_success << " / " << DEBUG_total << "-----------------------------\n";
    //YEN::globalsucc = DEBUG_success;
    //YEN::globaltotal = DEBUG_total;
  }
}
