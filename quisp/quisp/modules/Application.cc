...
void Application::initialize() {
  ...
  // just one connection with static S-D pair
    if (traffic_pattern == 3) {
      int initiator_address = par("LoneInitiatorAddress");
      if (my_address == initiator_address) {
        int endnode_dest_address = 100002; //getOneRandomEndNodeAddress();
        EV_INFO << "Just one connection setup request will be sent from " << my_address << " to " << endnode_dest_address << "\n";
        ConnectionSetupRequest *pk = createConnectionSetupRequest(endnode_dest_address, number_of_resources);
        scheduleAt(simTime(), pk);
      }
      return;
    }
