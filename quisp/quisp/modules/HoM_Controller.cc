...
void HoMController::sendNotifiers(){
     ...
     try{
         ...
         error("Error in HoMController.cc. It does not have port named toRouter_port.");
         endSimulation();
...
void HoMController::handleMessage(cMessage *msg){
	...
	if(dynamic_cast<BSAstart *>(msg) != nullptr){
        ...
        if((int)(getParentModule()->par("address"))-10 == 22 && multattempt==MAXATTEMPTS){
            consumeLinks();
            multattempt = 0;
...
void HoMController::checkNeighborAddress(bool receiver){
    if(receiver){
        try{
            ...
            error("Error in HoMController.cc when getting neighbor addresses. Check internodeInitializer.");
            endSimulation();
        }
        ...
        try{
            ...
            error("Error in HoMController.cc. Your stand-alone HoM module may not connected to the neighboring node (quantum_port).");
            endSimulation();
        }
    }
...
void HoMController::checkNeighborBuffer(bool receiver){
    if(receiver){
        try{
            ...
            error("Error in HoMController.cc. HoM couldnt find parameter numBuffer in the neighbor's qnic.");
            endSimulation();
        }
    }else{
        try{
...
void HoMController::updateIDE_Parameter(bool receiver){
    try{
        ...
        error(e.what());
        //std::cout<<"E="<<e.what()<<"\n";
        //error("photon_detection_per_sec is missing as a HoMController parameter. Or maybe you should specify **.Speed_of_light_in_fiber = (number)km in .ini file.");
    }
}

//Generates a BSA timing notifier. This is also called only once for the same reason as sendNotifiers().
BSMtimingNotifier* HoMController::generateNotifier(double time, double speed_of_light_in_channel, double distance_to_neighbor, int destAddr, double accepted_burst_interval,int photon_detection_per_sec, int max_buffer){
...
CombinedBSAresults* HoMController::generateNotifier_c(double time, double speed_of_light_in_channel, double distance_to_neighbor, int destAddr, double accepted_burst_interval,int photon_detection_per_sec, int max_buffer){
...
double HoMController::calculateTimeToTravel(double distance, double c){
    return (distance/c);
}
...
int HoMController::getStoredBSAresultsSize(){
    return results.size();
}

int HoMController::entangledCount(){
...
void HoMController::printBSAresults(){
...
void HoMController::clearBSAresults(){
    results.clear();
}
...
void HoMController::setLinkState() {
    // Track link states of HoMs
    EV << "-------------------------------------------\n";
    int counter = 0;
    for(auto it : results ){
    ...
    if(thisaddr >= 10){
        ...
        if(multattempt==0){
        ...
        else if(multattempt < MAXATTEMPTS ){ //hard coded reattempt times
            ...
            for(auto it : results ){
                int index = it.first;
                bool entangled = it.second;
                if( (BUFFERSIZE-HoMstates[thisaddr])==index ){
                    HoMstates[thisaddr] += innerct;
                    break;
                }
                else if(HoMstates[thisaddr]==0){
                    HoMstates[thisaddr] +=innerct;
                }
                if(entangled) {
                    innerct++;
        ...
        if(thisaddr==22 && multattempt!=MAXATTEMPTS){
            std::cout << "-------------------ENTANGLEMENT ROUND NUMBER:  " << multattempt+1 << " !  \n";
            multattempt++;
        }
    }
    std::cout << "--------------------------------------\n";
    std::cout << "HoM number " << thisaddr << " has link state " << HoMstates[thisaddr] <<"!!! \n";
    std::cout << "Total attempted :  "<< results.size() << "\n";;
    for(auto it : results ){
        int index = it.first;
        bool entangled = it.second;
        std::cout<< entangled<<" ";
...
void HoMController::consumeLinks() {
...
void HoMController::emit_dist_and_cost(vector<int> path) {
...
bool HoMController::check_link_state(vector<int> segment){
...
std::vector<int> HoMController::recover(vector<int> path){
    std::cout <<"***********************************************************\n";
...
void HoMController::build_graph() {
